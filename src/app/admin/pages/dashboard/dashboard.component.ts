...
export class AdminDashboardComponent implements OnInit, AfterViewInit {
  ...
  getOverallCompletionPercentage(): number {
    const completed = this.completedProjectsCount;
  ...
  loadUsers() {
    ...
    this.http.get<any[]>(`${this.apiUrl}/users`, {
      ...
      next: (data) => {
        this.users = data;
        this.totalUsersCount = data.length; // Update count here
        this.isLoading = false;
      },
      error: (error) => {
  ...
  renderStatusChart(): void {
    ...
    if (this.statusChart) {
      this.statusChart.destroy(); // destroy previous chart if it exists
    }
    ...
    this.statusChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: [
            'rgba(255, 206, 86, 0.7)',    // yellow (To Do)
            'rgba(54, 162, 235, 0.7)',    // blue (In Progress)
            'rgba(75, 192, 192, 0.7)'     // green (Done)
          ],
          borderColor: [
            'rgba(255, 206, 86, 1)',
            'rgba(54, 162, 235, 1)',
